import 'package:flutter_application_didongnangcao/model/bill.dart';
...
class APIRepository {
  ...
  Future<bool> updateInfo( User user,
      String accountID, String token) async {
    try {
      final body = FormData.fromMap({
   ...
   Future<bool> removeAccount(
    ...
    try {
      final body =
          FormData.fromMap({ 'accountID': accountID});
      Response res = await api.sendRequest.delete('/WWAdmin/removeUser',
          options: Options(headers: header(token)), data: body);
      if (res.statusCode == 200) {
        print("ok remove product");
        return true;
